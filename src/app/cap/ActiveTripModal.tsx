...
export default function ActiveTripModal({ isOpen, onClose }: ActiveTripModalProps) {
        ...
        try {
            ...
            if (success) {
                ...
                if (typeof window !== 'undefined' && (window as any).sendToKotlin) {
                    ...
                    console.log('tamer 📱 tamer Sent status change to Flutter:', newStatus);

                } else {
                    console.log('tamer 📱 tamer Sent status change to Flutter failed :');
                }
                ...
                try {
                    ...
                    console.log('tamer 📡 Broadcasted status_changed event');
    ...
    const handleCompleteTrip = async (customerRating: number) => {
        if (!tripData) return;

        console.log('tamer 🏁 Completing trip with rating:', customerRating);

        try {
            // Send complete trip command to Flutter
            if (typeof window !== 'undefined' && (window as any).sendToKotlin) {
